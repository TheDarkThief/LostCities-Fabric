...
public class Spheres {
    ...
    public static void generateSpheres(LostCityTerrainFeature feature, ChunkRegion region, Chunk chunk) {
                    ...
                    for (int y = centery + 1; y < maxY; y++) {
                        ...
                        if (sqdist <= sqradius) {
                            ...
                            if (profile.CITYSPHERE_CLEARBELOW_UNTIL_AIR && bottom != Integer.MAX_VALUE) {
                                ...
                                driver.current(x, yy, z);
                                while (driver.getBlock() != (yy <= seaLevel ? feature.liquid : air) && yy > provider.getWorld().getBottomY()) {
                                    driver.block(yy <= outerSeaLevel ? feature.liquid : air);
                                    driver.decY();
                                    yy--;
                                }
